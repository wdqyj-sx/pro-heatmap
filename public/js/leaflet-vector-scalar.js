...
L.ScalarLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    onAdd: function onAdd (map) {
        ...
        // console.log(this._canvasLayer.options.pane)
        // let canvas = this._canvasLayer.options.pane.children[0];
        // console.log(canvas)
        this._map = map;
        //  this._map.on('mousemove',this._onMouseMove,this);
    },
    onAddC: function onAddC (mapC) {
...
// ]
    function hexToRgb (hex) {
        let arr = []
        arr.push(parseInt(hex.slice(2, 4), 16))
        arr.push(parseInt(hex.slice(4, 6), 16))
        arr.push(parseInt(hex.slice(6, 8), 16))
        return [0, arr]
    }
    ...
    //     [255.372, [103, 234, 67]], // 0 F
    //     [273.15, [158, 194, 25]], // 0 C
    ...
    //     [328, [144, 28, 79]]
    // ];
    var COLORSCALE = params.colorScale || colorTable.map(hexToRgb)
    var SCALE = {
    ...
    var setColorScale = function setColorScale () {
        ...
        let n = parseFloat(((MAX_VELOCITY_INTENSITY - MIN_VELOCITY_INTENSITY) / COLORSCALE.length).toFixed(4)),
